...
- No auth, no accounts, no database. **One server route**: `app/api/room/route.ts`, the WebSocket
  ...
  nothing**, reads nothing back, and is never contacted unless a URL carries `?room=`; every tool,
  every price and every export still runs in the visitor's tab. `main` still carries
  `output: "export"` and no route, and is where a static build comes from if one is ever wanted.
...
- MIT licence at the root; official AWS icons in `public/icons/aws/` with `NOTICE.md` carving them
  out of it. **`LICENSE` must stay the unmodified MIT text** · a note about the icons was appended
  to it, and GitHub's licence detector reported the repo as "Other", so the About sidebar never said
  MIT (found 2026-09-04 while running §18). The carve-out belongs in `NOTICE.md`, beside the files
  it is about, with the README pointing at it.
...
brought — no platform's permission, no partnership, no API to publish.
...
**Out of scope:** auth, stored state, live AWS account connection, EC2 hourly pricing, multi-region.
...
| **Execution** | One screen, no login, no account, no keys. Seeded sample with real containment. Undo/redo, full keyboard map, empty state, exports that `cdk synth`. |
...
| `share_link` | read | a URL that opens this drawing on the page · the document rides in the fragment (`iac/share.ts`), nothing is uploaded, the relay is not involved. Too long to send = say so and point at `export` json |
...
- **A drawing in a link** (`engine/iac/share.ts`, 2026-09-03 · the user asking whether "visualise my
  ...
  architecture" today, and it needs nothing stored.
- **Why nothing is stored** (2026-09-03, the user asking about Vercel Blob with a one-day
...
src/
  ...
  engine/iac/       share.ts (a drawing in a link · #doc / #template, nothing stored) ·
...
npm run build          # production build (kills the dev cache — restart dev after)
npm test               # vitest (244)
...
npx vercel deploy --prod --yes --scope joms3   # only when the user says deploy
                       # `--scope joms3` is required · the project is under that
                       # team and the CLI defaults to the personal account,
                       # where the deploy fails with "Not authorized"
```
