...
export function translateContainer(
  snap: Pick<StateSnapshot, "nodes" | "containers">,
  id: string,
  dx: number,
  dy: number,
): Pick<StateSnapshot, "nodes" | "containers"> {
  const ids = new Set([id, ...descendantIds(snap.containers, id)]);
  return {
    containers: snap.containers.map((c) =>
      ids.has(c.id) && c.bounds
        ? { ...c, bounds: { ...c.bounds, x: c.bounds.x + dx, y: c.bounds.y + dy } }
        : c,
    ),
    nodes: snap.nodes.map((n) =>
      n.container && ids.has(n.container)
